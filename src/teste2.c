...
void run_tests() {
    ...
    TFunc r1;
    r1.pk = 1;
    err = dbHashRead(funcFile, "funcionarios", &r1, sizeof(TFunc), pk_offset, next_pk_offset, generic_read, generic_write);
    ...
    TFunc r2;
    r2.pk = 2;
    err = dbHashRead(funcFile, "funcionarios", &r2, sizeof(TFunc), pk_offset, next_pk_offset, generic_read, generic_write);
    ...
    TFunc r3;
    r3.pk = 99;
    err = dbHashRead(funcFile, "funcionarios", &r3, sizeof(TFunc), pk_offset, next_pk_offset, generic_read, generic_write);
    ...
    TFunc f1_updated = TFunc_New(1, "Ana Silva", "11111111111", "ana.silva@email.com",
                                 ...
                                 "senhaNova123", 0, 0);
    err = dbHashUpdate(funcFile, "funcionarios", f1_updated.pk, &f1_updated, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    ...
    TFunc f99_updated = TFunc_New(99, "Inexistente", "99999999999", "naoexiste@email.com", 
                                  ...
                                  "senhaInvalida", 0, 0);
    err = dbHashUpdate(funcFile, "funcionarios", f99_updated.pk, &f99_updated, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    ...
    // Deletar f2
    err = dbHashDelete(funcFile, "funcionarios", 2, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    printf("Deleção f2: %s (código de erro: %d)\n", err == DB_OK ? "SUCESSO" : "FALHA", err);

    // Tentar deletar PK inexistente
    err = dbHashDelete(funcFile, "funcionarios", 99, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    printf("Deleção PK inexistente: %s (código de erro: %d)\n", err == DB_OK ? "FALHA" : "SUCESSO (ERRO ESPERADO)", err);

    // Tentar ler um registro deletado (f2)
    TFunc r2_deleted;
    r2_deleted.pk = 2;
    err = dbHashRead(funcFile, "funcionarios", &r2_deleted, sizeof(TFunc), pk_offset, next_pk_offset, generic_read, generic_write);
...
void run_tests2(){
    ...
    err = dbHashFind(funcFile, "funcionarios", &r1, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    printf("Leitura f1 (PK 1): %d - %s\n", err, err == DB_OK ? "SUCESSO" : "FALHA");
    TFunc_PrintGeneric(&r1);

    err = dbHashFind(funcFile, "funcionarios", &r2, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    ...
    // Deletar f1 (PK 1)
    err = dbHashDelete(funcFile, "funcionarios", 1, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    ...
    err = dbHashFind(funcFile, "funcionarios", &buffer1, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    printf("Leitura f1 (PK 1): %d - %s\n", err, err == DB_OK ? "SUCESSO" : "FALHA");
    TFunc_PrintGeneric(&r1);

    err = dbHashFind(funcFile, "funcionarios", &buffer2, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    printf("Leitura f2 (PK 63): %d - %s\n", err, err == DB_OK ? "SUCESSO" : "FALHA");
    TFunc_PrintGeneric(&r2);

    // Deletar f2 (PK 63)
    err = dbHashDelete(funcFile, "funcionarios", 63, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
    printf("Deleção f2 (PK 63): %d - %s\n", err, err == DB_OK ? "SUCESSO" : "FALHA");
...
void createUnorderedDatabase(FILE* file, int num_funcionarios, int num_trocas) {
    ...
    TFunc funcionario = {0};
    char nome_funcionario[BF_NOME];
    for (int i = 0; i < num_funcionarios; i++) {


        // Cria um nome variando com i
        sprintf(nome_funcionario, "Funcionario %d", i + 1); // Ex: "Funcionario 1", "Funcionario 2", etc.

        funcionario = TFunc_New(
            pks[i] + 1,
            nome_funcionario,      // Nome variado
        ...
        int err = dbHashAdd(file, "funcionarios", &funcionario, sizeof(TFunc), pk_offset, next_pk_offset, generic_read, generic_write);
...
int criaBase(){
    ...
    createUnorderedDatabase(funcFile, 70, 70); // Criar 100 funcionários
    ...
    dbHashFind(funcFile, "funcionarios", &f, sizeof(TFunc), pk_offset, next_pk_offset, status_offset, generic_read, generic_write);
...
void add_and_print_records(FILE* funcFile, size_t pk_offset, size_t next_pk_offset, size_t status_offset) {
    ...
    for (int i = 0; i < 10; i++) {
        int err = dbHashAdd(funcFile, "funcionarios", funcs[i], sizeof(TFunc), pk_offset, next_pk_offset, generic_read, generic_write);
        if (err != 0) {
